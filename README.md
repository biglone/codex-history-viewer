...
当前示例版本为 `v1.3.16`。如果 Release 仍是 draft 或仓库是 private，需要先执行 `gh auth login`，或设置有仓库读取权限的 `GH_TOKEN`。

#### 统一命令行安装入口

现在可以直接用一个脚本入口安装 `gui` 或 `cli`，不需要自己去 Release 页面找对应平台资产名。

- `install.sh`：支持 Linux / macOS
- `install.ps1`：支持 Windows
- `cli`：支持 macOS `arm64/x64`、Linux `x64/arm64`、Windows `x64`
- `gui`：支持 macOS `arm64/x64`、Linux `x64/arm64`、Windows `x64`

```bash
# Linux / macOS：安装最新 CLI
curl -fsSL https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install.sh | bash -s -- --target cli

# Linux / macOS：安装指定版本 GUI
curl -fsSL https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install.sh | bash -s -- --target gui --version v1.3.16
```

```powershell
# Windows PowerShell：安装最新 CLI
iwr https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install.ps1 -OutFile install.ps1
.\install.ps1 -Target cli -AddToPath

# Windows PowerShell：安装指定版本 GUI
.\install.ps1 -Target gui -Version v1.3.16
```

如果你更想区分使用 GUI / CLI 专用安装脚本，下面的两个入口仍然保留。

#### 桌面应用在线安装

Linux `x64/arm64` 会安装 AppImage 到 `~/.local/bin/codex-history-viewer` 并创建 desktop entry；macOS 会下载 DMG 并复制 `.app` 到 `/Applications`，无权限时改用 `~/Applications`；Windows 会下载并运行安装器。
...
# 安装指定版本
curl -fsSL https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install-gui.sh | VERSION=v1.3.16 bash
```

```powershell
# Windows PowerShell
iwr https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install-gui.ps1 -OutFile install-gui.ps1
.\install-gui.ps1 -Version v1.3.16
```
...
| Linux ARM64 | `codex-history-viewer-Linux-arm64.AppImage` / `.deb` / `.rpm` |
...
# 安装指定版本
curl -fsSL https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install-cli.sh | VERSION=v1.3.16 bash
```

```powershell
# Windows PowerShell
iwr https://raw.githubusercontent.com/biglone/codex-history-viewer/main/scripts/install-cli.ps1 -OutFile install-cli.ps1
.\install-cli.ps1 -Version v1.3.16 -AddToPath
```
